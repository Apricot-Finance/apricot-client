...
async function doPrice() {
  ...
  for (const poolConfig of config.getPoolConfigList()) {
   if (!poolConfig.isLp()) continue; // test raydium lp only
    ...
    try {
      ...
      const [, , lpAmt] = await priceInfo.fetchLRLpAmounts(tokId, conn);
      const [lVal, rVal] = await priceInfo.fetchLRStats(tokId, conn, true);
      const price2 = (lVal + rVal) / lpAmt;
